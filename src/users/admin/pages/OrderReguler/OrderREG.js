	...
	useEffect(
		() => {
			if (props.admin.token) {
				const payload = { token: props.admin.token };
				getOrderReguler(payload).then(res => {
					...
					{data.map((order, i) => {
						...
						return (
							<div className={classes.OrderCard} key={i}>
								...
								<p className={classes.ContentDate}>{moment(order.createdAt).format('D MMM YYYY')}</p>
								<p
								...
								<p
									...
									{props.isLoading && indexLoading === i ? <SpinnerCircle /> : order.status}
								</p>
...
const mapDispatchToProps = dispatch => {
	return {
		getOrderReguler: payload => dispatch(actionCreators.getOrderReguler(payload)),
