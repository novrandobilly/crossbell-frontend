...
const Invoice = props => {
	...
	useEffect(
		() => {
			if (props.auth.token) {
				getOrderInvoice({ orderId: orderid, token: props.auth.token }).then(res => {
					if (res) {
			...
			<React.Fragment>
				<div className={classes.Container}>
						...
						<table className={classes.Table}>
							...
							<tbody>
								<tr>
									...
									<th>
										Rp. {orderData.pricePerSlot ? orderData.pricePerSlot : orderData.price}
										,-
									</th>
									<th>
										Rp.{' '}
						...
						<div className={classes.Footer}>
							<div className={classes.CommentContainer}>
								...
								<div className={classes.CommentContent}>
									<ul>
										<li>
											Pembayaran harus dilakukan sebelum tanggal jatuh tempo yaitu 14 hari dari tanggal pemesanan.
										</li>
										<li>
											Pembayaran dapat di transfer ke rekening BCA{' '}
											<span style={{ fontWeight: '500' }}>1234567xxx</span> a/n Bagong
										</li>
										<li>
											Pembayaran melalui virtual account dapat transfer melalui bank BCA dengan nomor VA{' '}
					...
					<div style={{ display: 'flex', width: '100%', justifyContent: 'center' }}>
						<h2>
							<em>
								<Link style={{ textDecoration: 'none' }} to={`/co/${props.auth.userId}/listOrder`}>
									Selesai & lihat daftar order {'>>'}
								</Link>
							</em>
						</h2>
					</div>
